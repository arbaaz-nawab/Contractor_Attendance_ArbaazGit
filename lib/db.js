...
/**
 * PERMANENTLY delete a shift_log row (manager action from the Attendance tab).
 * The audit row goes into shift_log_deletions FIRST, so a failed audit write
 * aborts the delete instead of leaving an unrecorded deletion; if the delete
 * itself then fails, the audit row is removed again (best effort).
 * `shift` is a mapped row from getShiftById().
 */
export async function deleteShiftRowWithAudit(shift, deletedBy, deletedAt) {
  const supabase = getClient();
  const { data: audit, error: auditErr } = await supabase
    .from('shift_log_deletions')
    .insert({
      shift_id:      shift._row,
      engineer_name: shift['Engineer Name'],
      shift_date:    shift['Shift Date'],
      sign_in_time:  shift['Sign-In Time'],
      sign_out_time: shift['Sign-Out Time'],
      hours:         shift['Hours'],
      status:        shift['Status'],
      deleted_by:    deletedBy,
      deleted_at:    deletedAt,
      row_snapshot:  JSON.stringify(shift),
    })
    .select('id')
    .limit(1);
  if (auditErr) throw new Error(`Audit log write failed, nothing was deleted: ${auditErr.message}`);

  const { error } = await supabase.from('shift_log').delete().eq('id', shift._row);
  if (error) {
    if (audit && audit[0]) await supabase.from('shift_log_deletions').delete().eq('id', audit[0].id);
...
export async function getParkingBookings({ dateFrom, dateTo, status, company, search } = {}) {
  const data = await fetchAllRows((supabase) => {
    ...
    // 'Archived' = legacy Completed/Cancelled rows (no longer selectable in the UI, kept in the data).
    if (status === 'Archived') query = query.in('status', ['Completed', 'Cancelled']);
...
/** Company names already used on Planned Works rows (any week, including
 *  soft-deleted ones) — suggestion text only, same shape as
 *  getDistinctParkingCompanies(). */
export async function getDistinctPlannedWorksCompanies() {
  try {
    const data = await fetchAllRows((supabase) =>
      supabase.from('planned_works').select('id, company_name').order('id', { ascending: true })
    );
    return dedupeNames(data.map((r) => r.company_name));
  } catch {
    return [];
  }
}
